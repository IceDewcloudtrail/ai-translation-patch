...
def patch_saveload_fes_file(
    ...
    stage_block = make_battle_scene_title_block(b"$g_TMP", stage_pairs)
    ...
    payload = payload.replace(message_anchor, message_anchor + make_saveload_message_fix_block(), 1)
    report.append(f"{json_path.name}:chapter_select_titles\t{chapter_select_count}\ttranslated")
    report.append(f"{json_path.name}:battle_scene_titles\t{len(stage_pairs)}\tinserted")
...
def main() -> None:
    ...
    if not translated_dialogue.exists():
        raise PatchError(f"Missing translated dialogue dir: {translated_dialogue}")
    ...
    if not dialogue_jsons:
        raise PatchError(f"No translated dialogue JSON found in: {translated_dialogue}")
    if not qdb_jsons:
        raise PatchError(f"No translated QDB JSON found in: {translated_qdb}")
    if not saveload_json.exists():
    ...
    stage_pairs = make_stage_title_pairs(stage_for_title)
    ...
    if not config_text_src.exists():
        raise PatchError(f"Missing source FES: {config_text_src}")
    ...
    if not saveload_map.exists():
        raise PatchError(f"Missing saveload map: {saveload_map}")
    if not src_path.exists():
        raise PatchError(f"Missing source FES: {src_path}")
